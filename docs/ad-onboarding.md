...
- 연령대는 복수 선택한다. 구체적인 연령대와 `잘 모르겠어요`는 서로 배타적으로 disabled
  처리한다.
- 광고 목표는 모든 서비스 형태에 공통인 5개 선택지를 표시하고, `모바일 앱`과 `앱 + 웹 모두`에는
  `앱 설치`, `인앱 구매·행동`을 추가한다.
...
- 완료된 서비스 형태를 다른 값으로 수정하면 기존 광고 목표를 초기화하고 광고 목표 질문을 연다.
  목표를 다시 선택하면 다른 후속 답변을 유지한 채 원래 진행 단계로 복구한다.
