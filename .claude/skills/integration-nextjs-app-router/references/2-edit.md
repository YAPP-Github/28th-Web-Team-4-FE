...
Where possible, add calls for PostHog's identify() function on the client side upon logins and signups. Use only stable, non-sensitive identifiers from those flows (for example user ID or email), and never send passwords, OTPs, recovery codes, or other secrets. If there is server-side code, pass the client-side session and distinct ID to the server-side code to identify the user. On the server side, make sure events have a matching distinct ID where relevant.
